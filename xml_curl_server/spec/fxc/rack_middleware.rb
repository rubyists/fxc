...
describe "Dialplan Routing" do
  ...
  it "routes a dialplan request to /dialplan/context/" do
    user = FXC::User.find_or_create(username: "potato", extension: '2123')
    did = user.add_did(FXC::Did.new(:number => '2345678900'))
    did = user.add_target(FXC::Target.new(:value => '9994441234'))
...
describe "Directory Routing" do
  ...
  it "routes a register request to /directory/register/PROFILE/EXTENSION" do
    user = FXC::User.find_or_create(:username => "pacman", :extension => "2122", :active => true)
    ...
    last_response.body.should.include %{<user id="#{user.extension}"}
  end

  it "sends not found to /directory/register/PROFILE/EXTENSION if user is inactive" do
    user = FXC::User.find_or_create(:username => "unpacman", :extension => "3122", :active => false)
  ...
  it "routes a voicemail request to /directory/voicemail/PROFILE/EXTENSION" do
    user = FXC::User.find_or_create(:username => "pacman", :extension => "2122")
